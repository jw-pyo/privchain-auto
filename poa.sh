...
then
    ...
    then
        #execute geth node
        geth --datadir $DATADIR --syncmode 'full' --rpccorsdomain '*' --rpc --rpcaddr $IP --rpcport $RPCPORT --rpcapi 'personal,db,eth,web3,miner,txpool' --port $PORT --networkid 2018 --gasprice '0' --unlock $UNLOCK_ACCOUNTS --password $PASSWORD --mine console
    else
        #execute geth node
